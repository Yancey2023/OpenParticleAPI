package yancey.openparticle.api.run.math;
...
public class Matrix {
    ...
    public Matrix(DataInputStream dataInputStream) throws IOException {
        this.data = new float[16];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                this.data[j * 4 + i] = dataInputStream.readFloat();
            }
        }
    }
    ...
    public void writeToFile(DataOutputStream dataOutputStream) throws IOException {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                dataOutputStream.writeFloat(this.data[j * 4 + i]);
            }
        }
    }

    public static Matrix of(float[] data) {
        if (data.length != 16) {
...
//        return new Matrix(data);
    ...
    public Vec3 apply(Vec3 vec3) {
        if (vec3 == Vec3.ZERO) {
            return new Vec3(data[3], data[7], data[11]);
        }
